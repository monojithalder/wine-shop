...
namespace wineshop_off_shop
{
    public partial class daily_report : Form
    {
                ...
                if (rd.GetDateTime(1).Year == DateTime.Now.Year)
                {
                    if (rd.GetDateTime(1).Date == DateTime.Now.Date && rd.GetDateTime(1).Month == DateTime.Now.Month)
                    {
                        ...
                        while (rd2.Read())
                        {
                            ...
                            dt.Rows.Add(rd2.GetString(2),rd2.GetInt32(7), rd2.GetInt32(4), rd2.GetValue(3),rd2.GetString(5));
        ...
        public void pd_print(object obj, PrintPageEventArgs pe)
        {
            ...
            graphics.DrawString("QUANTITY", new Font("Courier New", 8), new SolidBrush(Color.Black), startX + 320, startY + Offset);
            ...
            graphics.DrawString(total_ml.ToString(), new Font("Courier New", 8), new SolidBrush(Color.Black), startX + 320, startY + Offset);
            graphics.DrawString(total_price.ToString("00.00"), new Font("Courier New", 8), new SolidBrush(Color.Black), startX + 420, startY + Offset);
            total = 0;
            //dt1.Clear();
